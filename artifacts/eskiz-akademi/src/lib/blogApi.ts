const API_BASE = import.meta.env.BASE_URL.replace(/\/$/, '').replace('/eskiz-akademi', '') + '/api-server/api';
...
export async function updatePost(slug: string, data: Partial<CreateBlogPostInput>): Promise<BlogPost> {
  const res = await fetch(`${API_BASE}/blog/posts/${slug}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(data),
